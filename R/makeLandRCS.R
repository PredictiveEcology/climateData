...
#' @param outptuDir file path to directory where raster stacks will be written
...
#'
#' @rdname makeLandRCS
makeLandRCS <- function(pathToNormalRasters, pathToFutureRasters, rasterPrefix, outputDir, origTemplate, years = 2011:2100) {
  ...
  writeRaster(ATAstack, filename = file.path(outputDir, paste0(rasterPrefix, "_ATA", years[1], '-', tail(years, 1), ".grd")), datatype = "INT2S")
  writeRaster(CMIstack, filename = file.path(outputDir, paste0(rasterPrefix, "_CMI", years[1], '-', tail(years, 1), ".grd")), datatype = "INT2S")
}
